...
class PLANNER():
    def __init__(self, path):
        ...
        print(self.config_list)              ############PRINT###############3
        ...
        self.teacher = autogen.AssistantAgent(
            name="teacher",
            is_termination_msg=termination_msg,
            system_message="You are an expert engineer and python coder capable of teaching any relevent topic."
            "Reply `TERMINATE` in the end when everything is done.",
            llm_config=self.llm_config,
        )
        ...
        self.reviewer = autogen.AssistantAgent(
            ...
            is_termination_msg=termination_msg,
            system_message="You review the contents and intent of documents or files. Only review one document at a time. Ask for help, if needed. Reply `TERMINATE` in the end when everything is done.",
            llm_config=self.llm_config,
        )
    ...
    def rag_chat(self, PROBLEM):
        ...
        groupchat = autogen.GroupChat(
            agents=[self.boss_aid, self.coder, self.pm, self.reviewer, self.teacher, self.planner, self.planner_user], messages=[], max_round=12
        )
    ...
    def call_rag_chat(self, PROBLEM):
        ...
        for agent in [self.coder, self.pm, self.reviewer, self.teacher, self.planner]:
        ...
        groupchat = autogen.GroupChat(
            agents=[self.boss, self.boss_aid, self.coder, self.pm, self.reviewer, self.teacher, self.planner, self.planner_user], messages=[], max_round=12
        )
